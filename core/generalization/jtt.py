...
def train_stage_2(config, old_model, cur_model, loader, test_loader, device, fp16_run=False):
    ...
    for epoch in range(n_epochs):
        ...
        acc = evaluate(cur_model, test_loader, device)
...
def train():
    ...
    model_1.load_state_dict(torch.load('log/jtt/model_stage_1.pth'))
    model_2 = get_model(model_name=config['model_name'], num_classes=num_classes)
    print('Train stage 2')
    model_2 = train_stage_2(config, model_1, model_2, train_loader, test_loader, device, fp16_run=fp16_run)
